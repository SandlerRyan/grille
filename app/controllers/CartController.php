...
class CartController extends \BaseController {
    ...
    */
    public function increment_addon($addon_id, $item_id)
    {
        ...
        if (!$insertion){ 
            return json_encode(array('status' => 'success', 'validated' => 'false')); 
        }
        else {
            $total = Cart::total_with_addons();
            $response_array['status'] = 'success';    
    ...
    */
    public function decrement_addon($addon_id, $item_id)
    {
        ...
        // check if item exists and if associated addon exists; if not, do nothing
        if($item){
            $addon = Cart::find_addon($addon_id, $item);    // cart object in session
            if($addon) {
                if($addon->quantity == 1){
